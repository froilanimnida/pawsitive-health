...
import { ArrowLeft, Tag, Award } from "lucide-react";
...
export async function generateMetadata({ params }: UUIDPageParams): Promise<Metadata> {
    ...
    return {
        title: `${post.title} | Veterinary Forum | PawsitiveHealth`,
        description: post.content.substring(0, 160),
    };
}

// Post page component
export default async function VetForumPostPage({ params }: UUIDPageParams) {
    const { uuid } = await params;
    ...
    // Check if the author is a veterinarian
    const isVet = post.users?.role === "veterinarian";
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="text-sm text-muted-foreground">
                                {isVet ? "Veterinary Professional" : "Community Member"}
                            </div>
                        </div>
                    </div>
